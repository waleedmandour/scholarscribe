...
{#if comparison}
  ...
  {#if comparison?.draft}
    <h2>Reading-level metrics (draft)</h2>
    <div class="card">
      <div class="card-title">Standard readability scores</div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td><strong>{fmt(comparison.draft.flesch_reading_ease, 1)}</strong></td>
            <td class="muted">{fleschLabel(comparison.draft.flesch_reading_ease)}</td>
          </tr>
          <tr>
            <td>Flesch-Kincaid Grade Level</td>
            <td><strong>{fmt(comparison.draft.flesch_kincaid_grade, 1)}</strong></td>
            <td class="muted">US grade level equivalent</td>
          </tr>
          <tr>
            <td>Gunning Fog Index</td>
            <td><strong>{fmt(comparison.draft.gunning_fog, 1)}</strong></td>
            <td class="muted">Years of formal education needed</td>
          </tr>
          <tr>
            <td>Avg. syllables per word</td>
            <td><strong>{fmt(comparison.draft.avg_syllables_per_word, 2)}</strong></td>
            <td class="muted">Lower = simpler vocabulary</td>
          </tr>
          <tr>
            <td>Complex-word ratio (3+ syllables)</td>
            <td><strong>{(comparison.draft.complex_word_ratio * 100).toFixed(1)}%</strong></td>
            <td class="muted">Share of complex words</td>
          </tr>
        </tbody>
      </table>
    </div>
