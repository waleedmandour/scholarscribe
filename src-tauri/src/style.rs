...
pub fn analyze(text: &str) -> StyleProfile {
    ...
    let syllables_f = total_syllables as f64;
    let complex_f = complex_words as f64;

    let flesch_reading_ease = if words_f == 0.0 || sentences_f == 0.0 {
        0.0
    } else {
        206.835 - 1.015 * (words_f / sentences_f) - 84.6 * (syllables_f / words_f)
    };
    let flesch_kincaid_grade = if words_f == 0.0 || sentences_f == 0.0 {
        0.0
    } else {
        0.39 * (words_f / sentences_f) + 11.8 * (syllables_f / words_f) - 15.59
    };
    let gunning_fog = if words_f == 0.0 || sentences_f == 0.0 {
        0.0
    } else {
        0.4 * ((words_f / sentences_f) + 100.0 * (complex_f / words_f))
    };
